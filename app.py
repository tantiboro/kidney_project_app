...
st.title("📊 Interactive Modeler (V5)")  # <-- V5 MARKER
...
if uploaded_file is not None:
    ...
    if df is not None:
        st.info(
            "Displaying default dataset. Upload a new CSV to analyze your own data."
        )
...
if df is None:
    ...
    if features and target:
        ...
        with plot_col_1:
            ...
            if feature_to_plot:
                try:
                    fig1 = plotting.plot_feature_distribution(
                        df, feature_to_plot, target
                    )
                    st.plotly_chart(
                        fig1, width="stretch"
                    )  # --- FIX for deprecation ---
        ...
        if model_results["status"] == "error":
            ...
            if new_data is not None:
                prediction = model.get_prediction(
                    model_results["model"], model_results["scaler"], new_data
                )
                st.success(
                    f"The predicted value for the target is: **{prediction[0]}**"
                )
